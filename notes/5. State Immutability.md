...
- state의 내부 값 수정해서 state return
  ...
  - ObjectUseState에서는 객체 내부 값을 바꿔도 전혀 console.log가 출력되지 않는 것과, console.log 보면 초기 값 `{fname: 'Bruce', lname: 'Wayne'}`에서 첫번째 버튼 누르면 `{fname: 'Clark', lname: 'Wayne'}`이 출력되고 두번째 버튼 누르면 `{fname: 'Bruce', lname: 'Kent'}` 이 된다 
